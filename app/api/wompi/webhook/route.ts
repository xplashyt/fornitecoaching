...
import { isZonaClaraReference, parseReference } from "@/lib/orders";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!transaction || typeof transaction.reference !== "string" || !isZonaClaraReference(transaction.reference)) return NextResponse.json({ received: true });
    ...
    if (event.event === "transaction.updated" && transaction.status === "APPROVED" && matchesOrder && !reportedTransactions.has(transaction.id)) {
      ...
      console.log("[VENTA PAGADA]", JSON.stringify({
        curso: order.plan.name,
        referencia: transaction.reference,
        transaccion: transaction.id,
        correo: transaction.customer_email,
        monto: transaction.amount_in_cents,
        medio: transaction.payment_method_type ?? transaction.payment_method?.type,
        soporte: CORREO_CONTACTO
      }));
    }
