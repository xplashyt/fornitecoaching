...
export function parseReference(reference: string) {
  ...
  if (!timestamp || !/^\d+$/.test(timestamp) || !planId.startsWith(REFERENCE_PREFIX)) return null;
  const plan = getPlan(planId);
  return plan ? { plan, timestamp: Number(timestamp) } : null;
}

export function isZonaClaraReference(reference: string) {
