...
export function getServerWompiConfig() {
  ...
  if (!privateKey || !integritySecret || !eventsSecret) {
    throw new Error("Faltan las llaves privadas de Wompi en el servidor.");
  }
  ...
  if (new Set(environments).size !== 1) {
    throw new Error("Las cuatro llaves de Wompi deben pertenecer al mismo ambiente.");
  }
  ...
  return {
    ...
    integritySecret,
    eventsSecret,
    baseUrl: wompiBaseUrl(publicKey),
