...
interface AcceptanceResponse {
  data?: { presigned_acceptance?: AcceptanceItem; presigned_personal_data_auth?: AcceptanceItem; };
}
...
function hasAcceptanceTokens(data: AcceptanceResponse["data"]): data is NonNullable<AcceptanceResponse["data"]> & { presigned_acceptance: AcceptanceItem; presigned_personal_data_auth: AcceptanceItem } {
  return Boolean(
    ...
    data.presigned_acceptance.permalink &&
    data.presigned_personal_data_auth?.acceptance_token &&
    data.presigned_personal_data_auth.permalink
  );
}
...
export async function getAcceptanceTokens() {
  ...
  return {
    ...
    links: {
      terms: payload.data.presigned_acceptance.permalink,
      personalData: payload.data.presigned_personal_data_auth.permalink
    } satisfies PublicAcceptanceLinks
  };
}
